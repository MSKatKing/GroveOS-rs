...
impl PageTable {
    pub fn new() -> &'static mut Self {
        ...
        unsafe {
            (page.leak().0.as_ptr() as *mut PageTable)
                .as_mut()
    ...
    pub fn get_or_insert_with<F: FnOnce() -> &'static mut PageTable>(
        &mut self,
        idx: usize,
        f: F,
    ) -> &mut PageTable {
        ...
        unsafe {
            (self.0[idx].get_addr() as *mut PageTable)
                .as_mut()
                .expect("should not be null")
        }
    }
    ...
    pub fn identity_map(&mut self, virt: u64) {
        self.get_mut(virt).map_to(virt).set_writable(true);
    }
}
